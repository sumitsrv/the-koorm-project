...
private fun demonstrateMVPFeatures(assistant: OCPDAssistantManager) {
    ...
    val writeReportTask = assistant.createTask(
        ...
        dueDate = Clock.System.now().plus(DateTimePeriod(days = 3), TimeZone.currentSystemDefault())
    )
    ...
    val preferredTime = Clock.System.now().plus(DateTimePeriod(hours = 2), TimeZone.currentSystemDefault())
    val timeBlock = assistant.scheduleTask(writeReportTask.id, preferredTime)

    if (timeBlock != null) {
        val startTime = timeBlock.startTime.toLocalDateTime(TimeZone.currentSystemDefault())
        val endTime = timeBlock.endTime.toLocalDateTime(TimeZone.currentSystemDefault())
        println("📅 Scheduled: ${timeBlock.title}")
        println("   ⏰ ${startTime.time} - ${endTime.time}")
